...
push!(LOAD_PATH, "./resonance-knowledge/src/") 
push!(LOAD_PATH, ".") 
using Chakra, Resonances, DataSource, DataFrames, CSV
...
function nodeCSV()
    df_node = DataFrame(Id=drsIds_labels[1], Level=3)

    # Add DynR node
    push!(df_node, (dynrIds_labels[1], 1))

    # Add Harmonic nodes
    ld = length(harmonicIds_labels) - 1 # -1 due to useless noise batch 
    for i in 1:ld
        push!(df_node, (harmonicIds_labels[i], 2)) 
    end

    resStart = 0
    l = length(sliceIds_labels) # 380
    for i in 1:l
        push!(df_node, (sliceIds_labels[i], 4)) #3 previously

        for j in 1:length(pairIds_labels[i][:])
            push!(df_node, (pairIds_labels[i][j], 5)) # 44, 41, 47, 55, ...

            lk = length(resonanceIds_labels[j+resStart][:])
            for k in 1:lk
                push!(df_node, (resonanceIds_labels[j+resStart][k], 6))
            end
        end
...
function edgeCSV()
    ...
    for i in 1:lh
        push!(df_edge, (dynrIds_labels[1], harmonicIds_labels[i]))
